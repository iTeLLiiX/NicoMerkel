...
function initPageTransitions() {
  ...
  // Check if Barba.js is available (not available when running from file://)
  if (typeof barba === 'undefined') {
    // Initialize without Barba.js for local file usage
...
/**
* Anchor Navigation (Locomotive aware)
*/
function initAnchorLinks() {
  ...
  links.forEach(link => {
    ...
    link.addEventListener('click', (event) => {
      // Wenn wir bereits auf der Startseite sind und Ziel existiert -> smooth scroll via Locomotive
      const onHome = location.pathname.endsWith('/') || location.pathname.endsWith('index.html') || location.pathname === '';
      const target = document.getElementById(targetId);

      if (onHome && target) {
        event.preventDefault();
        if (scroll && scroll.scrollTo) {
          scroll.scrollTo(target, { offset: -40, duration: 800, easing: [0.25, 0.1, 0.25, 1] });
        } else {
      ...
      // Wenn wir auf einer Unterseite sind und nur ein Hash-Link klicken, gehe auf index.html mit Hash
      if (!onHome && (rawHref.startsWith('#') || url.pathname.endsWith('index.html') || url.pathname.endsWith('/'))) {
        event.preventDefault();
